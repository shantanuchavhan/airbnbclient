  ...
  bookings.map((booking, index) => (
    <div key={index} className="Trip">
      {booking.listing?.photos && (
        <img
          onClick={() => ViewListing(booking.listing)}
          className="Trip__image"
          src={booking.listing.photos[0]}
          alt=""
        />
      )}
      <h3 onClick={() => ViewListing(booking.listing)} className="Trip__Title">
        {booking.listing ? booking.listing.title : 'Listing Not Found'}
      </h3>
      <h4 className="Trip__division">
      ...
      <div
        className="listing_data Trip__division"
        onClick={() => setIsDeleteBox(index)}
      >
        <svg
      ...
      {isDeleteBox === index ? (
        <DeleteBox
          deleteAction={() => deleteBooking(booking._id)}
